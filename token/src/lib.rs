//! Access control.
//!
//! Access control in Binix is simple and stateless [0] - The server validates
//! the JWT against the configured key and allows access based on the `https://jwt.binix.rs/v1`
//! claim.
//!
//! One primary goal of the Binix Server is easy scalability. It's designed
...
//!
//! ```text
//! machine binix.server.tld password eyJhb...
//! ```
//!
//! ## Example token
//!
...
use binix::cache::{CacheName, CacheNamePattern};

/// Custom claim namespace for the BinixAccess information.
///
...
pub const CLAIM_NAMESPACE: &str = "https://jwt.binix.rs/v1";
...
struct TokenClaims {
    /// Binix namespace.
    #[serde(rename = "https://jwt.binix.rs/v1")]
    binix_ns: BinixAccess,
}

/// Permissions granted to a client.
///
/// This is the content of the `binix-access` claim in JWTs.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct BinixAccess {
    /// Cache permissions.
    ///
...
impl Token {
    ...
    pub fn get_or_insert_permission_mut(
        ...
        let access = self.binix_access_mut();
    ...
    fn binix_access(&self) -> &BinixAccess {
        &self.0.custom.binix_ns
    }

    fn binix_access_mut(&mut self) -> &mut BinixAccess {
        &mut self.0.custom.binix_ns
    }
}
