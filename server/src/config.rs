...
///
/// This will be concatenated into `$XDG_CONFIG_HOME/binix`.
const XDG_PREFIX: &str = "binix";
...
const ENV_CONFIG_BASE64: &str = "BINIX_SERVER_CONFIG_BASE64";

/// Environment variable storing the base64-encoded HMAC secret (used for signing and verifying
/// received JWTs).
const ENV_TOKEN_HS256_SECRET_BASE64: &str = "BINIX_SERVER_TOKEN_HS256_SECRET_BASE64";

/// Environment variable storing the base64-encoded RSA PEM PKCS1 private key (used for signing and
/// verifying received JWTs).
const ENV_TOKEN_RS256_SECRET_BASE64: &str = "BINIX_SERVER_TOKEN_RS256_SECRET_BASE64";

/// Environment variable storing the base64-encoded RSA PEM PKCS1 public key (used for verifying
/// received JWTs only).
const ENV_TOKEN_RS256_PUBKEY_BASE64: &str = "BINIX_SERVER_TOKEN_RS256_PUBKEY_BASE64";

/// Environment variable storing the database connection string.
const ENV_DATABASE_URL: &str = "BINIX_SERVER_DATABASE_URL";

/// Configuration for the Binix Server.
...
pub struct Config {
    ...
    /// be insecure.
    ///
    /// The API endpoint _must_ end with a slash (e.g., `https://domain.tld/binix/`
    /// not `https://domain.tld/binix`).
...
///
/// `binixadm test-chunking` provides a way to test chunking
...
pub struct CompressionConfig {
    ...
    /// Compression level.
    ///
    /// If unspecified, Binix will choose a default one.
...
pub struct GarbageCollectionConfig {
    ...
    /// it can still be run manually with `binixd --mode garbage-collector-once`.
