...
async fn main() -> Result<()> {
    ...
    match opts.mode {
        ServerMode::Monolithic => {
            binix_server::run_migrations(config.clone()).await?;

            let (api_server, _) = join!(
                binix_server::run_api_server(opts.listen, config.clone()),
                binix_server::gc::run_garbage_collection(config.clone()),
            );

            api_server?;
        }
        ServerMode::ApiServer => {
            binix_server::run_api_server(opts.listen, config).await?;
        }
        ServerMode::GarbageCollector => {
            binix_server::gc::run_garbage_collection(config.clone()).await;
        }
        ServerMode::DbMigrations => {
            binix_server::run_migrations(config).await?;
        }
        ServerMode::GarbageCollectorOnce => {
            binix_server::gc::run_garbage_collection_once(config).await?;
        }
...
fn dump_version() {
    ...
    eprintln!("Binix Server {} (debug)", env!("CARGO_PKG_VERSION"));
