...
use binix::api::binary_cache::BINIX_CACHE_VISIBILITY;
...
/// Sets the `X-Binix-Cache-Visibility` header in responses.
pub(crate) async fn set_visibility_header(
    ...
    if req_state.public_cache.load(Ordering::Relaxed) {
        response
            .headers_mut()
            .append(BINIX_CACHE_VISIBILITY, HeaderValue::from_static("public"));
    }
