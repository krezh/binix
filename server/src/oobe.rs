...
//!
//! This performs automatic setup for people running `binixd`
//! directly without specifying any configurations. The goal is
//! to let them quickly have a taste of Binix with a config
//! template that provide guidance for them to achieve a more
//! permanent setup.
//!
//! Paths:
//! - Config: `~/.config/binix/server.yaml`
//! - SQLite: `~/.local/share/binix/server.db`
//! - NARs: `~/.local/share/binix/storage`
...
use crate::config;
use binix::cache::CacheNamePattern;
...
pub async fn run_oobe() -> Result<()> {
    ...
    eprintln!();
    eprintln!("    binix login local http://localhost:8080 {root_token}");
    eprintln!();
    eprintln!("Documentations and guides:");
    eprintln!();
    eprintln!("    https://docs.binix.rs");
    eprintln!();
    eprintln!("Enjoy!");
    eprintln!("-----------------");
    eprintln!();
