...
use crate::error::{BinixError, BinixResult};
use crate::hash::Hash;
...
impl StorePath {
    ...
    fn from_base_name(base_name: PathBuf) -> BinixResult<Self> {
        let s = base_name
            .as_os_str()
            .to_str()
            .ok_or_else(|| BinixError::InvalidStorePathName {
                base_name: base_name.clone(),
                reason: "Name contains non-UTF-8 characters",
            })?;

        if !STORE_BASE_NAME_REGEX.is_match(s) {
            return Err(BinixError::InvalidStorePathName {
                base_name,
...
impl StorePathHash {
    ...
    pub fn new(hash: String) -> BinixResult<Self> {
        if hash.as_bytes().len() != STORE_PATH_HASH_LEN {
            return Err(BinixError::InvalidStorePathHash {
                hash,
...
#[cfg_attr(not(feature = "nix_store"), allow(dead_code))]
fn to_base_name(store_dir: &Path, path: &Path) -> BinixResult<PathBuf> {
    if let Ok(remaining) = path.strip_prefix(store_dir) {
        let first = remaining
            .iter()
            .next()
            .ok_or_else(|| BinixError::InvalidStorePath {
                path: path.to_owned(),
                reason: "Path is store directory itself",
            })?;

        if first.len() < STORE_PATH_HASH_LEN {
            Err(BinixError::InvalidStorePath {
                path: path.to_owned(),
