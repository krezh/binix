...
impl NixStore {
    pub fn connect() -> BinixResult<Self> {
        #[allow(unsafe_code)]
    ...
    /// follows symlinks.
    pub fn follow_store_path<P: AsRef<Path>>(&self, path: P) -> BinixResult<StorePath> {
    ...
    pub fn parse_store_path<P: AsRef<Path>>(&self, path: P) -> BinixResult<StorePath> {
    ...
    pub async fn query_path_info(&self, store_path: StorePath) -> BinixResult<ValidPathInfo> {
        let inner = self.inner.clone();
